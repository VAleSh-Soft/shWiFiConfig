...
```
где
- `HTTP` - ссылка на экземпляр Web-сервера, с которым будет работать конфиг;
...
```
установка интервала проверки состояния подключения (в милисекундах); интервал по умолчанию - 30 секунд; второй метод - соответственно, получение текущего интервала4
```
void setLogOnState(bool log_on);

bool getLogOnState();
```
включение/отключение выдачи сообщений о работе модуля через `Serial`; второй метод - соответственно, получение текущего состояния опции;
```
...
Остальные настройки доступны через Web-интерфейс модуля.
