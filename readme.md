...
+ добавлен контроль длины `SSID` и пароля `WiFI` с помощью методов `setApSsid()` и т.д.;
+ добавлен контроль длины имени и пароля администратора для доступа к Web-интерфейсу при использовании метода `setAdminData()`;
+ добавлен метод `setAdditionalEepromSize(uint16_t _add_eeprom_size)`, позволяющий выделить дополнительный размер памяти под `EEPROM`;
...
```
выделение дополнительного размера памяти под `EEPROM`; параметр `_add_eeprom_size` - размер дополнительной памяти, не может превышать `3070` байт (подробнее см. [здесь](#работа-с-eeprom))

Вызывать метод `setAdditionalEepromSize()` следует после вызова метода `begin()`.
...
Однако, если вы планируете использовать `EEPROM` для хранения и других данных, вы можете выделить для этого дополнительный размер памяти. Для этого используется метод `void setAdditionalEepromSize(uint16_t _add_eeprom_size)`, где `_add_eeprom_size` - размер дополнительно выделяемой под `EEPROM` памяти.
